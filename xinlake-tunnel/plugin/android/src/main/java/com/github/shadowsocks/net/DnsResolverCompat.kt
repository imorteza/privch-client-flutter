...
sealed class DnsResolverCompat {
    ...
    private object DnsResolverCompat23 : DnsResolverCompat() {
        ...
        private val unboundedIO by lazy {
            if (TunnelCore.instance().activityManager.isLowRamDevice) Dispatchers.IO
        ...
        private suspend fun resolveRaw(
            query: ByteArray, networkSpecified: Boolean = true,
            ...
            val isIpv6 = when (val type = question?.type) {
                ...
                Type.PTR -> {
                    ...
                    return prepareDnsResponse(request).apply {
                        hostname?.let {
                            addRecord(PTRRecord(question.name, DClass.IN, TTL, it), Section.ANSWER)
                        }
                    }.toWire()
                }
    ...
    private object DnsResolverCompat29 : DnsResolverCompat(), Executor {
        ...
        private val activeNetwork
            get() = TunnelCore.instance().connectivityManager.activeNetwork ?: throw IOException("no network")
