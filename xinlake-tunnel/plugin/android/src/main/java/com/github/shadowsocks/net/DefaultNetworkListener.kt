...
object DefaultNetworkListener {
    ...
    suspend fun get() = if (fallback) @TargetApi(23) {
        ...
        TunnelCore.instance().connectivityManager.activeNetwork ?: throw UnknownHostException()
    ...
    private fun register() {
        when (Build.VERSION.SDK_INT) {
            in 31..Int.MAX_VALUE -> @TargetApi(31) {
                TunnelCore.instance().connectivityManager.registerBestMatchingNetworkCallback(
                    request,
                    Callback,
                    mainHandler)
            }
            in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                TunnelCore.instance().connectivityManager.requestNetwork(
                    request,
                    Callback,
                    mainHandler)
            }
            in 26 until 28 -> @TargetApi(26) {
                TunnelCore.instance().connectivityManager.registerDefaultNetworkCallback(
                    Callback,
                    mainHandler)
            }
            in 24 until 26 -> @TargetApi(24) {
                TunnelCore.instance().connectivityManager.registerDefaultNetworkCallback(Callback)
            }
            else -> try {
                fallback = false
                TunnelCore.instance().connectivityManager.requestNetwork(request, Callback)
    ...
    private fun unregister() = TunnelCore.instance().connectivityManager.unregisterNetworkCallback(Callback)
}
