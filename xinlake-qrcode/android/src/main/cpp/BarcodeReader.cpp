...
static jobject CreateContentType(JNIEnv *env, ContentType contentType) {
    ...
    jfieldID fidCT = env->GetStaticFieldID(cls, JavaContentTypeName(contentType),
...
//		LOGD("time: %4d ms\n", (int)std::chrono::duration_cast<std::chrono::milliseconds>(duration).count());
        ...
        if (!results.empty()) {
            ...
            env->SetByteArrayRegion(jByteArray, 0, res.bytes().size(),
                                    (jbyte *) res.bytes().data());
            ...
            jfieldID fidContentType = env->GetFieldID(clResult, "contentType",
                                                      "Lcom/zxingcpp/BarcodeReader$ContentType;");
            env->SetObjectField(result, fidContentType, CreateContentType(env, res.contentType()));

            jfieldID fidPosition = env->GetFieldID(clResult, "position",
            ...
            jfieldID fidSymbologyIdentifier = env->GetFieldID(clResult, "symbologyIdentifier",
                                                              "Ljava/lang/String;");
            env->SetObjectField(result, fidSymbologyIdentifier,
                                C2JString(env, res.symbologyIdentifier()));
...
Java_com_zxingcpp_BarcodeReader_readYBuffer(
        ...
        jstring formats, jboolean tryHarder, jboolean tryRotate, jboolean tryInvert,
        jboolean tryDownscale,
        jobject result) {
...
Java_com_zxingcpp_BarcodeReader_readBitmap(
    ...
    switch (bmInfo.format) {
        ...
        case ANDROID_BITMAP_FORMAT_RGBA_8888:
            ...
            break;
        default:
            return ThrowJavaException(env, "Unsupported format");
    }
    ...
    auto image = ImageView{pixels, (int) bmInfo.width, (int) bmInfo.height, fmt,
                           (int) bmInfo.stride}
